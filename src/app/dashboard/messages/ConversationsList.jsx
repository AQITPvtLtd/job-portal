...
export default function ConversationsList({
    ...
    filter,
    onFilterChange,
    refreshTrigger // New prop for force refresh
}) {
    const [threads, setThreads] = useState([]);
    const [searchQuery, setSearchQuery] = useState("");
    const [loading, setLoading] = useState(true);
    ...
    useEffect(() => {
        ...
        const interval = setInterval(loadThreads, 10000);
        return () => clearInterval(interval);
    }, [filter, refreshTrigger]); // Add refreshTrigger to dependency
                        ...
                        <div className="absolute top-full left-0 right-0 mt-2 bg-white border-2 border-gray-200 rounded-lg shadow-xl z-50 overflow-hidden animate-slide-down">
                            {filterOptions.map((option) => (
                                <button
                                    ...
                                    className={`w-full flex items-center gap-3 px-4 py-3 hover:bg-gradient-to-r hover:from-blue-50 hover:to-purple-50 transition-all ${filter === option.value ? 'bg-gradient-to-r from-blue-100 to-purple-100 font-semibold' : ''
                                        }`}
                                >
                                    <option.icon size={18} className={option.color} />
                                    <span className="text-gray-800">{option.label}</span>
                    ...
                    <div className="divide-y divide-gray-100">
                        {filteredThreads.map(t => (
                            <div
                                key={t.unique_id}
                                onClick={() => onSelectConversation(t.unique_id)}
                                className={`p-4 hover:bg-gradient-to-r hover:from-blue-50 hover:to-purple-50 cursor-pointer transition-all ${selectedUniqueId === t.unique_id
                                        ? 'bg-gradient-to-r from-blue-100 to-purple-100 border-r-4 border-r-blue-600'
                                        : 'border-r-4 border-r-transparent'
                                    }`}
                            >
                                <div className="flex gap-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex justify-between items-start mb-1">
                                            <div className="flex-1 min-w-0">
                                                <h3 className={`font-semibold text-sm truncate ${t.unread > 0 ? 'text-gray-900' : 'text-gray-800'
                                                    }`}>
                                                    {t.name || "Unknown User"}
                                                </h3>
                                                {t.job_title && (
                                        ...
                                        <div className="flex items-center gap-2 mt-1.5">
                                            ...
                                            <span className={`inline-block px-2 py-0.5 text-xs rounded-full font-medium ${t.role === 'employer'
                                                    ? 'bg-gradient-to-r from-purple-100 to-pink-100 text-purple-700'
                                                    : 'bg-gradient-to-r from-green-100 to-teal-100 text-green-700'
                                                }`}>
                                                {t.role === 'employer' ? '👔 Employer' : '👤 Seeker'}
